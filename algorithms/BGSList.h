...
class BGSListDataWithF {
public:
	BGSListDataWithF() {}
	BGSListDataWithF(const state &theData, double fCost, double gCost, double hCost, uint64_t parent, uint64_t parentlist, uint64_t openLoc, dataLocation location)
	:data(theData), f(fCost), g(gCost), h(hCost), parentID(parent), parentList(parentlist), openLocation(openLoc), where(location) { reopened = false; }
	state data;
	double f;
	double g;
	double h;
	uint64_t parentID;
    uint64_t parentList;  // 0 for f (default) and 1 for g
...
public:
	BGSListData() {}
	BGSListData(const state &theData, double gCost, double hCost, uint64_t parent, uint64_t parentlist, uint64_t openLoc, dataLocation location)
	:data(theData), g(gCost), h(hCost), parentID(parent), parentList(parentlist), openLocation(openLoc), where(location) { reopened = false; }
	state data;
	double g;
	double h;
	uint64_t parentID;
...
template<typename state, typename CmpKey, class dataStructure = BGSListData<state> >
class BGSList {
public:
	BGSList();
	~BGSList();
	...
	uint64_t AddOpenNode(const state &val, uint64_t hash, double f, double g, double h, uint64_t parentlist, uint64_t parent=kTAStarNoNode);
	uint64_t AddOpenNode(const state &val, uint64_t hash, double g, double h, uint64_t parentlist, uint64_t parent=kTAStarNoNode);
	uint64_t AddClosedNode(state &val, uint64_t hash, double f, double g, double h,uint64_t parentlist, uint64_t parent=kTAStarNoNode);
	uint64_t AddClosedNode(state &val, uint64_t hash, double g, double h, uint64_t parentlist, uint64_t parent=kTAStarNoNode);
...
BGSList<state, CmpKey, dataStructure>::BGSList()
{
}
 ...
 */
template<typename state, typename CmpKey, class dataStructure>
void BGSList<state, CmpKey, dataStructure>::Reset(int)
{
	table.clear();
	elements.clear();
	theHeap.resize(0);
 ...
 */
template<typename state, typename CmpKey, class dataStructure>
uint64_t BGSList<state, CmpKey, dataStructure>::AddOpenNode(const state &val, uint64_t hash, double f, double g, double h, uint64_t parentlist, uint64_t parent)
{
	...
	elements.push_back(dataStructure(val, f, g, h, parent, parentlist, theHeap.size(), kOpenList));
 ...
 */
template<typename state, typename CmpKey, class dataStructure>
uint64_t BGSList<state, CmpKey, dataStructure>::AddOpenNode(const state &val, uint64_t hash, double g, double h, uint64_t parentlist, uint64_t parent)
{
	//size_t hash = hashFcn(val);
	// should do lookup here...
	if (table.find(hash) != table.end())
	...
	elements.push_back(dataStructure(val, g, h, parent, parentlist,theHeap.size(), kOpenList));
	if (parent == kTAStarNoNode)
		elements.back().parentID = elements.size()-1;
	table[hash] = elements.size()-1; // hashing to element list location
	theHeap.push_back(elements.size()-1); // adding element id to back of heap
 ...
 */
template<typename state, typename CmpKey, class dataStructure>
uint64_t BGSList<state, CmpKey, dataStructure>::AddClosedNode(state &val, uint64_t hash, double f, double g, double h, uint64_t parentlist,uint64_t parent)
{
	// should do lookup here...
	assert(table.find(hash) == table.end());
	elements.push_back(dataStructure(val, f, g, h, parent,parentlist, 0, kClosedList));
	if (parent == kTAStarNoNode)
		elements.back().parentID = elements.size()-1;
	table[hash] = elements.size()-1; // hashing to element list location
	return elements.size()-1;
...
template<typename state, typename CmpKey, class dataStructure>
uint64_t BGSList<state, CmpKey, dataStructure>::AddClosedNode(state &val, uint64_t hash, double g, double h, uint64_t parentlist,uint64_t parent)
{
	// should do lookup here...
	assert(table.find(hash) == table.end());
	elements.push_back(dataStructure(val, g, h, parent, parentlist,0, kClosedList));
	if (parent == kTAStarNoNode)
		elements.back().parentID = elements.size()-1;
	table[hash] = elements.size()-1; // hashing to element list location
	return elements.size()-1;
 ...
 */
template<typename state, typename CmpKey, class dataStructure>
void BGSList<state, CmpKey, dataStructure>::Remove(uint64_t hash)
{
 ...
 */
template<typename state, typename CmpKey, class dataStructure>
void BGSList<state, CmpKey, dataStructure>::KeyChanged(uint64_t val)
{
 ...
 */
template<typename state, typename CmpKey, class dataStructure>
dataLocation BGSList<state, CmpKey, dataStructure>::Lookup(uint64_t hashKey, uint64_t &objKey) const
{
 ...
 */
template<typename state, typename CmpKey, class dataStructure>
uint64_t BGSList<state, CmpKey, dataStructure>::Peek() const
{
	assert(OpenSize() != 0);
 ...
 */
template<typename state, typename CmpKey, class dataStructure>
uint64_t BGSList<state, CmpKey, dataStructure>::Close(uint64_t objKey)
{
	assert(OpenSize() != 0);
 ...
 */
template<typename state, typename CmpKey, class dataStructure>
uint64_t BGSList<state, CmpKey, dataStructure>::Close()
{
	assert(OpenSize() != 0);
 ...
 */
template<typename state, typename CmpKey, class dataStructure>
void BGSList<state, CmpKey, dataStructure>::Reopen(uint64_t objKey)
{
 ...
 */
template<typename state, typename CmpKey, class dataStructure>
bool BGSList<state, CmpKey, dataStructure>::HeapifyUp(unsigned int index)
{
...
template<typename state, typename CmpKey, class dataStructure>
void BGSList<state, CmpKey, dataStructure>::HeapifyDown(unsigned int index)
{
	CmpKey compare;
